...
void CCBot::SaveReplay(std::string path)
{
...
void CCBot::OnGameEnd()
{
    ...
    std::string name = "/home/pitlabubuntu1/Replays/TvT/" + result;
    m_predictionManager.onGameEnd(name);
    SaveReplay(name + ".SC2Replay");
}
...
void CCBot::OnStep()
{
    ...
    m_gameCommander.setState(m_predictionManager.getState());
...
void CCBot::SetEnemyRace(std::string race)
{
    m_predictionManager.SetEnemyRace(race);
}
...
int CCBot::GetArmyCount() const
{
#ifdef SC2API
    return Observation()->GetArmyCount();
#endif
}
